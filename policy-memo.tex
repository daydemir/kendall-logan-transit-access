...
\title{Improving Kendall Airport Connectivity\\\vspace{0.5em}\large Policy Memo for Kendall Square Association}
...
\textbf{Airport Problem 1: Over capacity and growing}. The airport has already well-surpassed its original design capacity, and at a modest air travel growth rate of 2--3\% per year Logan will reach maximum capacity by the mid-2030s \parencite{faa2024}. Besides this, the capacity of the roads and transit systems that lead to Logan are also stretched. If Logan is to remain a usable airport past the next decade, significant investments will need to be made to managing capacity of access and of the airport itself.
...
The KSA should work with the MBTA to accelerate the electrification of all commuter rail lines, but especially the Fitchburg Line. This will reduce emissions and reduce travel times by up to 20\% \parencite{transitmatters2021}. This will provide commuters to the Kendall area with better non-automotive options for getting into Boston (and thus Kendall). Along with electrification, the MBTA should also gradually increase frequencies, aiming to provide 15 minute frequencies.
...
This branch would be an order of magnitude jump in the catchment area for transit access to Kendall. This will only increase the gravitational pull of businesses, startups, conferences, and events to Kendall.

The KSA should work with the MBTA and MIT to begin the process of determining a location for a Grand Junction Station that can become a branched second endpoint for the Fitchburg Line, other than North Station. As part of developing this branch, the line should be grade separated in order to avoid the friction that exists at the road crossings. While this require building a westbound turn onto the Fitchburg line and does not leverage the existing rail, electrification would need updated rail infrastructure and that opportunity should be taken to reuse the right-of-way for a grade separated, elevated electrified connection.
...
We take for granted that a new Boston airport will need to be built within the next 50 years. With this new airport, there are opportunities to avoid the mistakes that were made when building the last airport over 100 years ago.
